    ...
    enum Encoder {
        ...
        public static func encode(_ node: ASN1.Node) -> [UInt8] {
            ...
            case .integer(let integer):
                return IDENTIFIERS.INTEGER.bytes + asn1LengthPrefixed([UInt8](integer))
            case .bitString(let bits):
                return IDENTIFIERS.BITSTRING.bytes + asn1LengthPrefixed([0x00] + [UInt8](bits))
            case .octetString(let octet):
                return IDENTIFIERS.OCTETSTRING.bytes + asn1LengthPrefixed([UInt8](octet))
            case .null:
                return IDENTIFIERS.NULL.bytes
            case .objectIdentifier(let oid):
                return IDENTIFIERS.OBJECTID.bytes + asn1LengthPrefixed([UInt8](oid))
            case .ecObject(let ecObj):
                return IDENTIFIERS.EC_OBJECT.bytes + asn1LengthPrefixed([UInt8](ecObj))
            case .ecBits(let ecBits):
                return IDENTIFIERS.EC_BITS.bytes + asn1LengthPrefixed([UInt8](ecBits))
