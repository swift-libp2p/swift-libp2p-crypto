...
struct RSAPublicKey:CommonPublicKey {
    ...
    func encrypt(data: Data) throws -> Data {
        ...
        guard let encryptedData = SecKeyCreateEncryptedData(self.key, .rsaEncryptionPKCS1, data as CFData, &error) else {
...
struct RSAPrivateKey:CommonPrivateKey {
    ...
    func decrypt(data: Data) throws -> Data {
        var error:Unmanaged<CFError>?
        guard let decryptedData = SecKeyCreateDecryptedData(self.key, .rsaEncryptionPKCS1, data as CFData, &error) else {
